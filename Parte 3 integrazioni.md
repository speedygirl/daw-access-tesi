...
- Cambio Tasti, fornisce un feedback sonoro all'utente quando vengono invertiti gli stati dei tasti funzione o del capslock, num lock, scroll lock
...
In ognuno di questi è possibile aggiungere i metadati per UI Automation, ma per implementare funzionalità più complesse, spesso è richiesto che oltre ai componenti standard vengano utilizzate le API native di UI Automation implementare feature, più avanzate e non gestite dai controlli base offerti dalla piattaforma.

Uno dei punti di forza di Microsoft windows è il pacchetto di Microsoft Office, che essendo sviluppato da Microsoft ha delle funzionalità dedicate all'accessibilità estremamente avanzate, sopratutto nella sua ultima edizione, Office 2019. Inoltre, per Windows e Office, Microsoft offre un supporto di assistenza h24 dedicato all'accessibilità, così l'utente che non riesce ad affrontare un passaggio o un controllo, ha la possibilità di farsi guidare da un operatore e riportare in tempo reale i dati utili per la segnalazione di un eventuale bug: in questo modo si crea un corridoio preferenziale per permettere la riparazione dei problemi di accesso.

Nelle ultime release di Windows 10 è possibile utilizzare le barre braille con BRLTTY, il famosissimo driver braille GNU GPL sviluppato su Linux, già base degli screen reader Orca su Linux e NVDA su windows e sta diventando lo standard anche per Windows Narrator.
