...
Tra i sistemi operativi commerciali per personal computer windows è il più diffuso come numero di installazioni, nella sua storia ormai più che ventennale ha subito innumerevoli evoluzioni e trasformazioni, oggi su piattaforma x64 ha il più grande parco di supporti e periferiche, tra queste possiamo trovare anche diverse tipologie di periferiche dedicate all'accessibilità supportati per lo più da software di terze parti, ad ogni modo in questo capitolo ci interesseremo alle funzionalità di accessibilità integrate all'interno della edizione Windows 10, andando a delimitare quali sono le sue competenze e il ruolo che gioca nell'esperienza utente.

Nel linguaggio Microsoft le funzioni di accessibilità vengono definite "Ease of Access" in italiano sarebbe "Accesso Facilitato", queste funzionalità sono di tipo eterogeneo e tutte influenzano l'interazione con l'utente, alcune agiscono a livello di comandi di input, altre aggiungono riferimenti visivi o uditivi, generalmente queste impostazioni vengono gestite dal pannello di controllo, anche se possono essere attivate tramite specifiche scorciatoie predefinite, quando queste funzionalità vengono attivate le applicazioni devono essere in grado di gestirle per questo Windows in fase di ecuzione mette a disposizione un elenco di parametri che le applicazioni possono leggere per modificare il loro comportamento o disabilitare\abilitare funzionalità e comportamenti potenzialmente dannosi per l'esperienza utente (ad esempio un applicazione potrebbe cambiare il modo in cui gestisce il focus da tastiera quando è attivo uno screenreader), tra questi parametri troviamo: [^win-access-params]
- Contrasto Elevato, indica alle applicazioni di impostare una combinazione di colori ad alto contrasto
- Preferenza Tastiera, comunica che l'utente preferisce ad accedere utilizzando la tastiera invece del mouse o del touch, l'applicazione potrebbe aver bisogno di mostrare dei controlli dedicati all'accesso da tastiera
...
Queste configurazioni delegano completamente all'applicazione l'implementazione e la gestione dei vari casi d'uso, ci sono però alcune feature di accessibilità che il sistema operativo può attivare su richiesta dell'utente o dell'applicazione.
